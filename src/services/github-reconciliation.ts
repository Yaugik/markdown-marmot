...
const PARSER_VERSION = "legacy-marked-v1";

type BranchContext = {
  workspaceId:string;projectId:string;selectedBranchId:string;repositoryLinkId:string;
  branchName:string;branchRevision:number;rulesVersion:number;parserVersion:string;
  includeRules:string[];excludeRules:string[];linkState:string;branchState:string;
  repositoryState:string;installationState:string;providerInstallationId:number;
  ownerLogin:string;repositoryName:string;activeSnapshotId:string|null;actorPrincipalId:string;
};
...
type ParsedFile = {
  ...
  markdown:string;renderedHtml:string;plainText:string;headings:Array<{level:number;text:string;slug:string;position:number}>;
};
export type GitReconciliationResult = {
  selectedBranchId:string;snapshotId:string|null;headOid:string|null;published:boolean;
  requeued:boolean;fileCount:number;added:number;changed:number;removed:number;unchanged:number;
  warningCount:number;
};
...
function providerFailure(error:unknown):never{
  if(error instanceof GitHubProviderError){
    throw new FoundationServiceError("CONFLICT",error.message,{providerCode:error.code,retryable:error.retryable});
  }
  throw error;
}
...
async function claimBranch(pool:Pool,selectedBranchId:string,workerId:string):Promise<BranchContext>{
  return workerTransaction(pool,async(client)=>{
    const result=await client.query<{
      workspace_id:string;project_id:string;id:string;repository_link_id:string;branch_name:string;
      revision:string;rules_version:string;parser_version:string;include_rules:string[];exclude_rules:string[];
      link_state:string;branch_state:string;repository_state:string;installation_state:string;
      provider_installation_id:string;owner_login:string;repository_name:string;active_snapshot_id:string|null;
      created_by_principal_id:string;
    }>(`SELECT branch.workspace_id,branch.project_id,branch.id,branch.repository_link_id,branch.branch_name,
      branch.revision,link.rules_version,link.parser_version,link.include_rules,link.exclude_rules,
    ...
    if(!leased.rows[0])throw new FoundationServiceError("CONFLICT","Selected Git branch is already being reconciled.");
    return {
      workspaceId:row.workspace_id,projectId:row.project_id,selectedBranchId:row.id,
      repositoryLinkId:row.repository_link_id,branchName:row.branch_name,
      branchRevision:Number(row.revision),rulesVersion:Number(row.rules_version),
      parserVersion:row.parser_version,includeRules:row.include_rules,excludeRules:row.exclude_rules,
      linkState:row.link_state,branchState:row.branch_state,repositoryState:row.repository_state,
      installationState:row.installation_state,providerInstallationId:Number(row.provider_installation_id),
      ownerLogin:row.owner_login,repositoryName:row.repository_name,
      activeSnapshotId:row.active_snapshot_id,actorPrincipalId:row.created_by_principal_id,
    };
  });
}
...
async function enumerateTree(context:BranchContext,treeSha:string):Promise<TreeBlob[]>{
  ...
  while(queue.length){
    ...
    let page;try{page=await getGitHubTree(context.providerInstallationId,context.ownerLogin,context.repositoryName,item.sha,false);}catch(error){providerFailure(error);}
...
async function fetchFiles(context:BranchContext,entries:TreeBlob[]):Promise<{files:ParsedFile[];warnings:string[]}>{
  ...
  for(const entry of scoped.sort((left,right)=>left.path.localeCompare(right.path))){
    ...
    let blob;try{blob=await getGitHubBlob(context.providerInstallationId,context.ownerLogin,context.repositoryName,entry.sha);}catch(error){providerFailure(error);}
    ...
    files.push({
      id:newFolioId(),path:entry.path,blobOid:entry.sha,sizeBytes:entry.size,
...
async function stageCandidate(pool:Pool,context:BranchContext,headOid:string,files:ParsedFile[],warnings:string[],counts:{added:number;changed:number;removed:number;unchanged:number}){
  ...
  return workerTransaction(pool,async(client)=>{
    const existing=await client.query<{id:string;state:string;inventory_hash:string|null;file_count:number}>(`SELECT id,state,inventory_hash,file_count FROM git_snapshots
      WHERE selected_branch_id=$1 AND head_oid=$2 AND rules_version=$3 AND parser_version=$4 FOR UPDATE`,
    [context.selectedBranchId,headOid,context.rulesVersion,context.parserVersion]);
    if(existing.rows[0]){
      const row=existing.rows[0];
      if(row.state==="published"||row.state==="superseded")return{snapshotId:row.id,alreadyPublished:true};
...
async function discardCandidate(pool:Pool,snapshotId:string){
  await workerTransaction(pool,async(client)=>{await client.query(`UPDATE git_snapshots SET state='discarded',completed_at=now(),failure_code='HEAD_CHANGED',failure_message='Branch head changed during reconciliation' WHERE id=$1 AND state='candidate'`,[snapshotId]);});
}
...
async function publishCandidate(pool:Pool,context:BranchContext,workerId:string,snapshotId:string,headOid:string,files:ParsedFile[],counts:{added:number;changed:number;removed:number;unchanged:number},deliveryId?:string){
  return workerTransaction(pool,async(client)=>{
    ...
    if(!current||current.lease_owner!==workerId)throw new FoundationServiceError("CONFLICT","Git branch reconciliation lease was lost.");
    if(Number(current.rules_version)!==context.rulesVersion)throw new FoundationServiceError("CONFLICT","Markdown scope changed during reconciliation.");
    ...
    if(current.active_snapshot_id){
      await client.query(`UPDATE git_snapshots SET state='superseded',completed_at=coalesce(completed_at,now()) WHERE id=$1 AND state='published'`,[current.active_snapshot_id]);
    }
    await client.query(`UPDATE git_snapshots SET state='published',published_at=now(),completed_at=now() WHERE id=$1 AND state='candidate'`,[snapshotId]);

    const activePaths=new Set(files.map((file)=>file.path));
    for(const file of files){
      const existing=await client.query<{page_id:string}>(`SELECT page_id FROM git_pages WHERE selected_branch_id=$1 AND source_path=$2 FOR UPDATE`,[context.selectedBranchId,file.path]);
      const pageId=existing.rows[0]?.page_id??newFolioId();
      if(existing.rows[0]){
        await client.query(`UPDATE pages SET title=$4,status='active',archived_at=NULL,revision=revision+1,updated_by_principal_id=$5,updated_at=now()
          WHERE workspace_id=$1 AND project_id=$2 AND id=$3`,[context.workspaceId,context.projectId,pageId,file.title,context.actorPrincipalId]);
      }else{
        await client.query(`INSERT INTO pages(id,workspace_id,project_id,source_type,title,status,created_by_principal_id,updated_by_principal_id)
          VALUES($1,$2,$3,'git',$4,'active',$5,$5)`,[pageId,context.workspaceId,context.projectId,file.title,context.actorPrincipalId]);
      }
      await client.query(`INSERT INTO git_pages(page_id,workspace_id,project_id,selected_branch_id,source_path,
        ...
        ON CONFLICT(selected_branch_id,source_path) DO UPDATE SET current_snapshot_file_id=excluded.current_snapshot_file_id,
          current_head_oid=excluded.current_head_oid,current_blob_oid=excluded.current_blob_oid,
          content_hash=excluded.content_hash,markdown=excluded.markdown,rendered_html=excluded.rendered_html,
    ...
    const removed=await client.query<{page_id:string}>(`SELECT page_id FROM git_pages WHERE selected_branch_id=$1 AND NOT(source_path=ANY($2::text[]))`,[context.selectedBranchId,[...activePaths]]);
    for(const row of removed.rows){
      await client.query(`UPDATE pages SET status='unavailable',revision=revision+1,updated_by_principal_id=$4,updated_at=now()
        WHERE workspace_id=$1 AND project_id=$2 AND id=$3 AND status='active'`,[context.workspaceId,context.projectId,row.page_id,context.actorPrincipalId]);
      await client.query(`DELETE FROM page_search_documents WHERE workspace_id=$1 AND project_id=$2 AND page_id=$3 AND source_type='git'`,[context.workspaceId,context.projectId,row.page_id]);
    }
    const branchUpdate=await client.query<{revision:string}>(`UPDATE selected_git_branches SET active_snapshot_id=$2,last_observed_head_oid=$3,last_reconciled_at=now(),lease_owner=NULL,lease_expires_at=NULL,revision=revision+1,updated_at=now() WHERE id=$1 RETURNING revision`,[context.selectedBranchId,snapshotId,headOid]);
    ...
    await client.query(`INSERT INTO activity_events(id,workspace_id,project_id,actor_principal_id,authorizing_principal_id,source,action,target_type,target_id,input_summary,result_summary,request_id,trace_id)
    ...
    await client.query(`INSERT INTO outbox_events(id,workspace_id,project_id,aggregate_type,aggregate_id,aggregate_revision,event_type,actor_principal_id,authorizing_principal_id,request_id,trace_id,payload)
      VALUES($1,$2,$3,'selected_git_branch',$4,$5,'github.branch_reconciled.v1',$6,$6,$7,$8,$9)`,
    [outboxId,context.workspaceId,context.projectId,context.selectedBranchId,revision,context.actorPrincipalId,requestId,`github-reconcile:${snapshotId}`,{snapshotId,headOid,...counts}]);
...
export async function reconcileGitHubBranch(
  ...
  try{
    let ref;try{ref=await getGitHubRef(context.providerInstallationId,context.ownerLogin,context.repositoryName,`heads/${context.branchName}`);}catch(error){providerFailure(error);}
    const headOid=ref.data.object.sha;
    let commit;try{commit=await getGitHubCommit(context.providerInstallationId,context.ownerLogin,context.repositoryName,headOid);}catch(error){providerFailure(error);}
    ...
    const staged=await stageCandidate(pool,context,headOid,files,warnings,counts);snapshotId=staged.snapshotId;
    if(staged.alreadyPublished){await releaseBranch(pool,context.selectedBranchId,input.workerId);return{selectedBranchId:context.selectedBranchId,snapshotId,headOid,published:true,requeued:false,fileCount:files.length,...counts,warningCount:warnings.length};}
    let finalRef;try{finalRef=await getGitHubRef(context.providerInstallationId,context.ownerLogin,context.repositoryName,`heads/${context.branchName}`);}catch(error){providerFailure(error);}
    if(finalRef.data.object.sha!==headOid){
      await discardCandidate(pool,snapshotId);
      await workerTransaction(pool,async(client)=>{await client.query(`INSERT INTO jobs(id,workspace_id,project_id,kind,payload,deduplication_key,status,priority,max_attempts,available_at) VALUES($1,$2,$3,'github.branch.reconcile',$4,$5,'pending',50,8,now()) ON CONFLICT DO NOTHING`,[newFolioId(),context.workspaceId,context.projectId,{selectedBranchId:context.selectedBranchId,headHint:finalRef.data.object.sha},`github-reconcile:${context.selectedBranchId}:${finalRef.data.object.sha}`]);});
      await releaseBranch(pool,context.selectedBranchId,input.workerId);
      return{selectedBranchId:context.selectedBranchId,snapshotId,headOid,published:false,requeued:true,fileCount:files.length,...counts,warningCount:warnings.length};
    }
    await publishCandidate(pool,context,input.workerId,snapshotId,headOid,files,counts,input.deliveryId);
    return{selectedBranchId:context.selectedBranchId,snapshotId,headOid,published:true,requeued:false,fileCount:files.length,...counts,warningCount:warnings.length};
  }catch(error){
    if(snapshotId)await workerTransaction(pool,async(client)=>{await client.query(`UPDATE git_snapshots SET state='failed',completed_at=now(),failure_code=$2,failure_message=$3 WHERE id=$1 AND state='candidate'`,[snapshotId,error instanceof FoundationServiceError?error.code:"RECONCILIATION_FAILED",error instanceof Error?error.message.slice(0,500):"Reconciliation failed"]);}).catch(()=>undefined);
    if(input.deliveryId)await workerTransaction(pool,async(client)=>{await client.query(`UPDATE github_webhook_deliveries SET status='failed',processed_at=now(),last_error_code=$2,last_error_message=$3 WHERE delivery_id=$1`,[input.deliveryId,error instanceof FoundationServiceError?error.code:"RECONCILIATION_FAILED",error instanceof Error?error.message.slice(0,500):"Reconciliation failed"]);}).catch(()=>undefined);
    await releaseBranch(pool,context.selectedBranchId,input.workerId);
    throw error;
  }
}
