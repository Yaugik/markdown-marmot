...
import { listPageAttachments, preparePageAttachment } from "@/services/page-attachments";
...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const attachments = await listPageAttachments({
      workspaceId: scope.workspace_id,
      projectId: scope.project_id,
      pageId: validatedPageId,
    }, authenticated.session.principalId);
    return jsonSuccess(attachments, context);
  } catch (error) {
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await preparePageAttachment({
    ...
    return jsonSuccess({
      ...
      upload_path: `/api/v1/attachments/${result.data.id}/content`,
