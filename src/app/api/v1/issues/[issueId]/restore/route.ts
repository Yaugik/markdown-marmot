...
import { restoreIssue } from "@/services/issues";
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await restoreIssue({ workspaceId: input.workspace_id, projectId: input.project_id, issueId: id, expectedRevision: input.expected_revision }, mutationContext(authenticated.session.principalId, context, idempotencyKey));
