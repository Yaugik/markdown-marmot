...
import { removeIssueDependency } from "@/services/issue-relations";
...
export async function DELETE(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await removeIssueDependency({ workspaceId: input.workspace_id, projectId: input.project_id, dependencyId: id, expectedSourceRevision: input.expected_revision }, mutationContext(authenticated.session.principalId, context, idempotencyKey));
    return jsonSuccess({ dependency: { id: result.data.id, source_issue_id: result.data.sourceIssueId, target_issue_id: result.data.targetIssueId, relation_kind: result.data.relationKind, created_at: result.data.createdAt }, activity_id: result.activityId, outbox_event_id: result.outboxEventId, replayed: result.replayed }, context);
  } catch (error) {
