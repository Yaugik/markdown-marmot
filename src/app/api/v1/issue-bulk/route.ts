...
const requestSchema = z.discriminatedUnion("operation", [
  ...
  z.object({ operation: z.literal("transition"), targetStatusId: z.string().uuid() }).strict(),
  z.object({ operation: z.literal("archive") }).strict(),
  z.object({ operation: z.literal("restore") }).strict(),
]);
const schema = z.object({ workspace_id: z.string().uuid(), project_id: z.string().uuid(), issue_ids: z.array(z.string().uuid()).min(1).max(500), request: requestSchema }).strict();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await prepareIssueBulkOperation({ workspaceId: input.workspace_id, projectId: input.project_id, issueIds: input.issue_ids, request: input.request }, mutationContext(authenticated.session.principalId, context, idempotencyKey));
    return jsonSuccess({ preview: bulkPreviewResponse(result.data), activity_id: result.activityId, outbox_event_id: result.outboxEventId, replayed: result.replayed }, context, result.replayed ? 200 : 201);
  } catch (error) {
