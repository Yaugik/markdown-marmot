...
import { executeIssueBulkOperation } from "@/services/issue-bulk";
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await executeIssueBulkOperation({ workspaceId: input.workspace_id, projectId: input.project_id, previewId: id, expectedRevision: input.expected_revision }, mutationContext(authenticated.session.principalId, context, idempotencyKey));
    return jsonSuccess({ preview: bulkPreviewResponse(result.data), activity_id: result.activityId, outbox_event_id: result.outboxEventId, replayed: result.replayed }, context);
  } catch (error) {
