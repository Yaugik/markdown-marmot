...
import { updateIssueSavedView } from "@/services/issue-views";
...
const schema = z.object({ workspace_id: z.string().uuid(), project_id: z.string().uuid(), expected_revision: z.number().int().positive(), name: z.string().trim().min(1).max(120).optional(), visibility: z.enum(["private", "project"]).optional(), projection: z.enum(["list", "board", "timeline", "calendar"]).optional(), filters: filtersSchema.optional(), grouping: groupingSchema.optional(), ordering: orderingSchema.optional() }).strict();
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await updateIssueSavedView({ workspaceId: input.workspace_id, projectId: input.project_id, viewId: id, expectedRevision: input.expected_revision, name: input.name, visibility: input.visibility, projection: input.projection, filters: input.filters, grouping: input.grouping, ordering: input.ordering }, mutationContext(authenticated.session.principalId, context, idempotencyKey));
    return jsonSuccess({ view: { id: result.data.id, workspace_id: result.data.workspaceId, project_id: result.data.projectId, owner_principal_id: result.data.ownerPrincipalId, name: result.data.name, visibility: result.data.visibility, projection: result.data.projection, filters: result.data.filters, grouping: result.data.grouping, ordering: result.data.ordering, revision: result.data.revision, created_at: result.data.createdAt, updated_at: result.data.updatedAt }, activity_id: result.activityId, outbox_event_id: result.outboxEventId, replayed: result.replayed }, context);
  } catch (error) {
