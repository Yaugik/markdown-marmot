...
type Issue = {
  ...
  dependencies: Array<{ id: string; relation_kind: string }>;
};
type Workflow = {
  id: string;
  name: string;
  statuses: Array<{ id: string; name: string; color_key: string }>;
  transitions: Array<{
    id: string;
...
type Portfolio = {
  ...
  milestones: Array<{ id: string; name: string; target_on: string | null }>;
  cycles: Array<{ id: string; name: string; starts_on: string; ends_on: string }>;
  roadmaps: Array<{ id: string; name: string }>;
};
type SavedView = {
  id: string;
  name: string;
...
const priorityLabel = (priority: Priority) => priority === "no_priority"
  ? "No priority"
...
export function IssueWorkspace({ workspaceId, projectId }: { workspaceId?: string; projectId?: string }) {
  ...
  const issueMap = useMemo(() => new Map((projection?.issues ?? []).map((issue) => [issue.id, issue])), [projection]);
  const workflow = workflows.find((item) => item.id === selectedIssue?.workflow_id);
  const transitions = workflow?.transitions.filter((item) => item.from_status_id === selectedIssue?.status.id) ?? [];
  ...
  }, [workspaceId, projectId, projectionKind, searchQuery]);
  ...
  const refreshWorkspace = useCallback(async () => {
    ...
    setError(null);
    try {
      await Promise.all([refreshConfiguration(), loadProjection()]);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Could not load work management data");
    } finally {
      setBusy(false);
    }
  }, [scope, refreshConfiguration, loadProjection]);

  const refreshSelectedIssue = useCallback(async () => {
    ...
    try {
      const [issue, issueComments, issueAttachments] = await Promise.all([
        ...
        api<IssueComment[]>(`/api/v1/issues/${selectedIssueId}/comments?${scope}`),
  ...
  useEffect(() => { void refreshSelectedIssue(); }, [refreshSelectedIssue]);
  ...
  async function createIssue(event: React.FormEvent) {
    ...
    try {
      const result = await api<{ issue: Issue }>("/api/v1/issues", {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("issue") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
          title: newTitle,
          description: documentFromText(newDescription),
  ...
  async function transitionIssue(targetStatusId: string, requiresComment: boolean) {
    if (!workspaceId || !projectId || !selectedIssue) return;
    const transitionComment = requiresComment ? window.prompt("This transition requires a comment.")?.trim() : undefined;
    if (requiresComment && !transitionComment) return;
    setBusy(true);
    setError(null);
    try {
      const result = await api<{ issue: Issue }>(`/api/v1/issues/${selectedIssue.id}/transition`, {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("transition") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
  ...
  async function addComment(event: React.FormEvent) {
    event.preventDefault();
    if (!workspaceId || !projectId || !selectedIssue || !newComment.trim()) return;
    try {
      await api(`/api/v1/issues/${selectedIssue.id}/comments`, {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("issue-comment") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
          body: documentFromText(newComment),
        }),
      });
      setNewComment("");
  ...
  async function saveCurrentView(event: React.FormEvent) {
    event.preventDefault();
    if (!workspaceId || !projectId || !viewName.trim() || !scope) return;
    try {
      await api("/api/v1/issue-views", {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("view") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
          name: viewName,
  ...
  async function prepareBulkArchive() {
    ...
    try {
      const result = await api<{ preview: BulkPreview }>("/api/v1/issue-bulk", {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("bulk-preview") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
  ...
  async function executeBulk() {
    ...
    setError(null);
    try {
      if (bulkPreview.risk_level === "R2" && bulkPreview.confirmation_id) {
        await api(`/api/v1/issue-bulk/confirmations/${bulkPreview.confirmation_id}/approve`, {
          method: "POST",
          headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("bulk-approve") },
          body: JSON.stringify({
            workspace_id: workspaceId,
            project_id: projectId,
      ...
      const result = await api<{ preview: BulkPreview }>(`/api/v1/issue-bulk/${bulkPreview.id}/execute`, {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("bulk-execute") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
          expected_revision: bulkPreview.revision,
        }),
      });
      setBulkPreview(result.preview);
      setSelectedIds(new Set());
      await Promise.all([loadProjection(), refreshSelectedIssue()]);
    } catch (cause) {
  ...
  async function uploadAttachment(file?: File) {
    ...
    try {
      ...
      const prepared = await api<{ upload_url: string }>(`/api/v1/issues/${selectedIssue.id}/attachments`, {
        method: "POST",
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey("issue-attachment") },
        body: JSON.stringify({
          workspace_id: workspaceId,
          project_id: projectId,
          file_name: file.name,
  ...
  if (!workspaceId || !projectId) {
    return <div className={styles.setup}>
      ...
      <p>Open this screen with workspace and project UUIDs to manage workflows, issues, saved views, roadmaps, and bulk operations.</p>
      <form onSubmit={(event) => {
        event.preventDefault();
  ...
  const renderIssue = (issue: Issue) => <article
    className={`${styles.issueCard} ${selectedIssueId === issue.id ? styles.selectedCard : ""}`}
    key={issue.id}
  >
    <label className={styles.checkbox}>
      <input type="checkbox" checked={selectedIds.has(issue.id)} onChange={() => toggleSelected(issue.id)} />
      <span />
    </label>
  ...
  return <div className={styles.workspace}>
    <header className={styles.header}>
      <div>
        <span className={styles.eyebrow}>Project work</span>
        <h1>Work management</h1>
        <p>{projection?.total ?? 0} visible issues · {workflows.length} workflows</p>
      </div>
      <button className={styles.refresh} type="button" onClick={() => void refreshWorkspace()} disabled={busy}>
    ...
    <section className={styles.toolbar}>
      ...
      <div className={styles.projectionTabs}>
        ...
        ] as const).map(([kind, Icon, label]) => <button
          key={kind}
          type="button"
          className={projectionKind === kind ? styles.activeTab : ""}
          onClick={() => setProjectionKind(kind)}
        ><Icon size={15} />{label}</button>)}
      </div>
      {selectedIds.size > 0 && <button className={styles.bulkButton} type="button" onClick={() => void prepareBulkArchive()}>
        <Archive size={15} /> Preview archive ({selectedIds.size})
      </button>}
    </section>

    <section className={styles.savedViews}>
      <span>Saved views</span>
      {views.map((view) => <button type="button" key={view.id} onClick={() => {
        setProjectionKind(view.projection);
        void loadProjection(view.id);
      }}>{view.name}<small>{view.projection}</small></button>)}
      <form onSubmit={saveCurrentView}>
        ...
        <button type="submit" aria-label="Save view"><Save size={14} /></button>
      </form>
    </section>

    {bulkPreview && <section className={styles.bulkPreview}>
      <div>
        <strong>Bulk {bulkPreview.operation} preview</strong>
        <span>{bulkPreview.impact.accessible_count} accessible · {bulkPreview.impact.blocked_count} blocked · {bulkPreview.impact.unavailable_count} unavailable</span>
      </div>
      {bulkPreview.impact.warnings.map((warning) => <p key={warning}>{warning}</p>)}
      {bulkPreview.state === "prepared" && <button type="button" onClick={() => void executeBulk()}>
        {bulkPreview.risk_level === "R2" ? "Approve and execute" : "Execute"}
      </button>}
      {bulkPreview.result && <div className={styles.bulkResult}>
        <span>{bulkPreview.result.succeeded.length} succeeded</span>
        <span>{bulkPreview.result.failed.length} failed</span>
      </div>}
    </section>}
      ...
      <aside className={styles.leftPanel}>
        <form className={styles.createIssue} onSubmit={createIssue}>
          ...
          <select value={newPriority} onChange={(event) => setNewPriority(event.target.value as Priority)}>
            {(["no_priority", "urgent", "high", "medium", "low"] as Priority[]).map((priority) => <option key={priority} value={priority}>{priorityLabel(priority)}</option>)}
          </select>
      ...
      <main className={styles.projectionPanel}>
        {projectionKind === "board" ? <div className={styles.board}>
          {(projection?.groups ?? []).map((group) => <section key={group.key}>
            ...
            <div>{group.issueIds.map((id) => issueMap.get(id)).filter((issue): issue is Issue => Boolean(issue)).map(renderIssue)}</div>
          </section>)}
          ...
          {(projection?.timeline ?? []).map((item) => {
            ...
            return issue ? <button type="button" key={item.issue_id} onClick={() => setSelectedIssueId(item.issue_id)}>
              <strong>{issue.identifier} · {issue.title}</strong>
              <span>{item.start_on ?? "Unscheduled"}<ChevronRight size={14} />{item.due_on ?? "Open ended"}</span>
            </button> : null;
          })}
        </div> : projectionKind === "calendar" ? <div className={styles.calendarGrid}>
          {(projection?.calendar ?? []).map((day) => <section key={day.date}>
            <header>{day.date}</header>
            {day.starts.map((id) => <button type="button" key={`start-${id}`} onClick={() => setSelectedIssueId(id)}>Starts · {issueMap.get(id)?.identifier}</button>)}
            {day.due.map((id) => <button type="button" key={`due-${id}`} onClick={() => setSelectedIssueId(id)}>Due · {issueMap.get(id)?.identifier}</button>)}
          </section>)}
        </div> : <div className={styles.issueList}>{(projection?.issues ?? []).map(renderIssue)}</div>}
        {!projection?.issues.length && <div className={styles.empty}>
          <GitPullRequestArrow size={24} />
          <h3>No issues in this projection</h3>
          <p>Create an issue or adjust the search and saved-view filters.</p>
        </div>}
      </main>
      ...
      <aside className={styles.detailPanel}>
        {selectedIssue ? <>
          <div className={styles.detailTitle}>
            <small>{selectedIssue.identifier}</small>
            <h2>{selectedIssue.title}</h2>
            <span>{selectedIssue.status.name} · r{selectedIssue.revision}</span>
          </div>
          ...
          <section className={styles.transitions}>
            ...
            {transitions.length ? transitions.map((item) => <button
              type="button"
              key={item.id}
              onClick={() => void transitionIssue(item.to_status_id, item.requires_comment)}
              disabled={busy}
            >{item.name}<ChevronRight size={14} /></button>) : <p>No transitions available.</p>}
          </section>
          <section className={styles.comments}>
            ...
            <form onSubmit={addComment}>
              <textarea value={newComment} onChange={(event) => setNewComment(event.target.value)} placeholder="Add a comment" rows={3} />
              <button type="submit"><Send size={14} /> Send</button>
            </form>
          </section>
          <section className={styles.attachments}>
            <h3><Paperclip size={15} /> Attachments</h3>
            {attachments.map((attachment) => <a key={attachment.id} href={`/api/v1/issue-attachments/${attachment.id}/content?${scope}`}>
              <span>{attachment.file_name}</span>
              <small>{Math.ceil(attachment.size_bytes / 1024)} KB · {attachment.storage_state}</small>
            </a>)}
          ...
          <p>Inspect workflow state, dates, relationships, comments, and attachments.</p>
        </div>}
      </aside>
    </div>
  </div>;
